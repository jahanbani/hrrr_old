...
def parallel_interpolate(args):
    wind_speed_data, shifted_power_curves, wind_farms, dt = args
    return [
        [
            interpolate(wind_speed_data.loc[dt, w],
                        shifted_power_curves.loc[w])
            for w in wind_farms["Bus"]
        ]
    ]
...
def interpolate_data(dt, wind_farms, wind_speed_data, shifted_power_curves):
...
def calculate_pout_parallel(
    ...
    num_threads = 50  # You can adjust this number as needed
    with ThreadPoolExecutor(max_workers=num_threads) as executor:
        wind_power_data = list(
            tqdm(
                executor.map(
                    parallel_interpolate,
                    [
                        (wind_speed_data, shifted_power_curves, wind_farms, dt)
                        for dt in dts
                    ],
                ),
                total=len(dts),
                desc="Calculating Power Output",
            )
        )
        ...
        try:
            ...
            for SELK, SELV in SELECTORS.items():
                try:
                    ...
                    data[SELK].loc[dt +
                                   datetime.timedelta(minutes=15)] = np.nan
                    data[SELK].loc[dt +
                                   datetime.timedelta(minutes=30)] = np.nan
                    data[SELK].loc[dt +
                                   datetime.timedelta(minutes=45)] = np.nan
            ...
            for SELK, SELV in SELECTORS.items():
                try:
                    for u in gribs.select(name=SELV):
                        time_info = (
                            str(u).split(":")[-1].strip()
                        )  # Extract time info from the name
                        time_minutes = int(time_info.split()[0])
                        data_key = dt + \
                            datetime.timedelta(minutes=time_minutes)
