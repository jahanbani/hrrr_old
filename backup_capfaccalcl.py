...
WIND_OUTPUT_FILE = OUTDIR + "wind_power.xlsx"

SOLAR_WIND_SPEED_FN = OUTDIR + "solar_wind_speed.xlsx"
SOLAR_TMP_DATA_FN = OUTDIR + "solar_tmp_data.xlsx"
SOLAR_RAD_DATA_FN = OUTDIR + "solar_rad_data.xlsx"
SOLAR_VDD_DATA_FN = OUTDIR + "solar_vdd_data.xlsx"
SOLAR_VBD_DATA_FN = OUTDIR + "solar_vdd_data.xlsx"
SOLAR_OUTPUT_FILE = OUTDIR + "solar_power.csv"

# study year; which year to study
YEAR = 2020
DIR = r"/research/alij/"
START = pd.to_datetime("2019-12-31 00:00")
END = pd.to_datetime("2023-01-02 01:00")
...
latlons = pd.read_csv("../psse/grg-pssedata/Buses_with_lat_lon_for_visualization.csv")[
    colslatlons
]
...
if WindRead:
    ...
    wind_power[1].to_excel(WIND_OUTPUT_FILE)
...
if SolarRead:
    ...
    wind_speed_data[1].to_excel(SOLAR_WIND_SPEED_FN)
    solar_tmp_data[1].to_excel(SOLAR_TMP_DATA_FN)
    solar_rad_data[1].to_excel(SOLAR_RAD_DATA_FN)
    solar_vdd_data[1].to_excel(SOLAR_VDD_DATA_FN)
    solar_vbd_data[1].to_excel(SOLAR_VBD_DATA_FN)


wind_speed_data = pd.read_excel(SOLAR_WIND_SPEED_FN, index_col=0)
wind_speed_data[wind_speed_data < 0] = 0
solar_tmp_data = pd.read_excel(SOLAR_TMP_DATA_FN, index_col=0)
solar_tmp_data[solar_tmp_data < 0] = 0
solar_rad_data = pd.read_excel(SOLAR_RAD_DATA_FN, index_col=0)
solar_rad_data[solar_rad_data < 0] = 0
solar_vdd_data = pd.read_excel(SOLAR_VDD_DATA_FN, index_col=0)
solar_vdd_data[solar_vdd_data < 0] = 0
solar_vbd_data = pd.read_excel(SOLAR_VBD_DATA_FN, index_col=0)
